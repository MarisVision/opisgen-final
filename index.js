...
export default function Home() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setOutput(data.output || 'Greška u generiranju.');
    } catch (err) {
      setOutput('Došlo je do pogreške.');
    }
    setLoading(false);
  };
  ...
  return (
    <main style={{ background: '#121212', color: '#fff', minHeight: '100vh', padding: '2rem', textAlign: 'center' }}>
      ...
      <pre style={{ background: '#1e1e1e', padding: '1rem', borderRadius: '8px', marginTop: '2rem', whiteSpace: 'pre-wrap' }}>{output}</pre>
      <a href="https://ko-fi.com/opisgen" target="_blank" style={{ color: '#ffd700', display: 'inline-block', marginTop: '20px' }}>☕ Podrži nas putem Ko-fi</a>
    </main>
  );
}
